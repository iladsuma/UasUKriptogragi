...
public class Main {
	...
	private void keyGenerate(String key) {
        ...
        for (int i = 0; i < P.length; i++) {
            ...
            P[i] = xor(P[i], paddedKey.substring(j, j + 8));
            System.out.println("subkey " + (i + 1) + ": " + P[i]);
            j = (j + 8) % paddedKey.length();
        }
    }
	// round function
	...
	Main()
	{
		...
		for (int i = 0; i < 32; i++)
			modVal = modVal << 1;
		String cipherText = "8baf7aacb62ba6e7";
		String key = "c79Be7246";
        // y dan r bukan hexa jadi tidak bisa digunakan untuk hex ke binary pada long parselong, maka dari itu saya ganti dengan huruf ascii. iladsuma
        // maka dari itu saya Konversikan string ke representasi yang berbeda(ascii) sebelum menguraikannya dengan Long.parseLong. 
